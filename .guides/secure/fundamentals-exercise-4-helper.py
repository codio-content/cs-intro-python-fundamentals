...
def check_variable(file):
  ...
  with open(file, "r") as code_to_check:
    for line in code_to_check.readlines():
      if "my_float" in line and "=" in line and "#" not in line and "'''" not in line and '"""' not in line:
        correct_var = True
...
def print_var(file):
  ...
  with open(file, "r") as code_to_check:
    for line in code_to_check.readlines():
      if "print" in line and "my_float" in line and "#" not in line and "'''" not in line and '"""' not in line:
        print_variable = True
  return print_variable
  
if not check_variable(student_code):
  print("<h2>Test did not pass</h2>")
  print("Program did not declare the variable 'my_float'.")
